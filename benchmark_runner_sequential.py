...
def run_experiment(algorithm_name, budget_multiplier=1000):
    ...
    output_folder = f"{algorithm_name}_Sequential_results"
    ...
    for problem in suite:
        ...
        elif algorithm_name == "WCA":
            # 1. Calcular iteraciones (presupuesto / población)
            iterations = int(max_evals / NP)
            
            # 2. Calcular número de ríos (usualmente el 10% de la población o 4 fijo)
            n_rivers = max(1, int(NP * 0.1)) # Para NP=50, serán 5 ríos
            
            # 3. Instanciar TU clase WCA
            # Nota: Mapeamos los datos de BBOB a tus nombres de variables
            optimizer = WCA(
                num_agents=NP,          # Tu variable: num_agents
                iter=iterations,        # Tu variable: iter
                num_dim=dim,            # Tu variable: num_dim
                min_p=lower_bound,            # Tu variable: min_p
                max_p=upper_bound,            # Tu variable: max_p
                fitness=problem,        # Tu variable: fitness (Pasamos el problema de COCO)
                num_of_rivers=n_rivers, # Tu variable: num_of_rivers
                # Parámetros opcionales (puedes dejarlos por defecto o ajustarlos)
                min_distance=1e-5,      
                epsilon=0.01, # Ojo: epsilon bajo en BBOB ayuda a la precisión
                w=2.0
            )
...
if __name__ == '__main__':
    ...
    run_experiment("WCA", budget_multiplier=20)